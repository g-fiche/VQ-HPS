...
def pkl_to_npz_train(dataset_path):

    list_pkl = [p for p in Path(f"{dataset_path}").glob(f"**/*.pkl")]
    random.shuffle(list_pkl)
    split_idx = int(0.8 * len(list_pkl))
    list_train = list_pkl[:split_idx]
    list_valid = list_pkl[split_idx:]

    for split in ["train", "validation"]:
        save_path = os.path.join(dataset_path, f"{split}.npz")
        imgname_list = []
        gender_list = []
        root_orient_list = []
        pose_list = []
                ...
                for i in range(data["n_frames"]):
                    if data["good_frames_mask"][i]:
                        if i not in data["bboxes"]["invalid_idxs"]:
                            ...
                            scale = (
                                1.2 * max(bbox[2] - bbox[0], bbox[3] - bbox[1]) / 200
                            )
...
pkl_to_npz_train("datasets/EMDB")
